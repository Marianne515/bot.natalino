...
moderators = ["iMooseMoo","Marianne_123","SSSsunshineSSS","A.M.A.N.D.A_19"]
...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")  
        await self.highrise.walk_to(Position(17.0 , 19.75 , 5.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala") 
        if user.username in moderators:
         await self.highrise.chat(f"🚫{user.username} Moderador(a) da sala\n[Entrou na sala]")
         
        await self.highrise.send_whisper(user.id,f"🎄Bem Vindo(a) a Doação de Natal @{user.username} se prescisar de ajuda com meus comandos digite !help em meu privado ou no chat da sala.")
